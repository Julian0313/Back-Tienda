...
namespace Logica.Implementacion
{
    ...
    public class ClienteLogica : IClienteLogica
    {
        ...
        {
            var crearCliente = await _clienteRepo.ObtenerClienteIdAsync(cliente.idCliente);
            if (crearCliente != null)
            {
                return RespuestaErrores.RespuestaError<string>("Ya exsiste un cliente con id : " + cliente.idCliente);
            }
            await _clienteRepo.CrearClienteAsync(cliente);
            await _unidadTrabajo.GuardarCambiosAsync();

            return RespuestaErrores.RespuestaOkay<string>("Cliente creado correctamente id : " + cliente.idCliente);
        }
